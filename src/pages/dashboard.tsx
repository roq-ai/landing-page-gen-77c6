...
import { getLandingPages } from 'apiSdk/landing-pages';
import { LandingPageInterface } from 'interfaces/landing-page';
import { getTeamMembers } from 'apiSdk/team-members';
import { TeamMemberInterface } from 'interfaces/team-member';
import { getMarketingManagers } from 'apiSdk/marketing-managers';
import { MarketingManagerInterface } from 'interfaces/marketing-manager';

import { MarketingManagerListPage as ShowCaseList } from 'pages/marketing-managers';

function DashboardPage() {
  const { session } = useSession();

  const landingPageStat = useSWR<PaginatedInterface<LandingPageInterface>>(
    () => `/landing-pages`,
    () => getLandingPages({ limit: 1, offset: 0 }),
  );
  ...
  const marketingManagerStat = useSWR<PaginatedInterface<MarketingManagerInterface>>(
    () => `/marketing-managers`,
    () => getMarketingManagers({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Landing Pages',
      result: landingPageStat?.data?.totalCount,
      isLoading: landingPageStat.isLoading,
    ...
    },
    {
      ...
      result: marketingManagerStat?.data?.totalCount,
      isLoading: marketingManagerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Marketing Managers
                </Text>
                <NextLink href={`marketing-managers`} passHref legacyBehavior>
                  <Button
